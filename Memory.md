...
- **Current Phase:** Plan: 6 dashboard-UX/HR requests (nav gating, collapsible sidebar, theme + branding, profile reposition, universal attendance/performance, growth parity) — a separate track from the numbered Plan.md phases above, tackled as Phases 18a/18b/18c/19/20/21. **All six phases are now done and verified live.**
- **Status (20, performance half + route move):** Complete and verified live. `lib/performance-data.ts` fully widened to a `PerformanceSubject` (`{kind:"employee"|"account", id}`) via a `subjectWhereFor(subject)` helper, the same shape `attendance-data.ts` adopted for its half: `scoreInputsFor`, `loadPeriodScore`, `loadPerformanceBreakdown`, `loadPerformanceHistory`, `loadGoals`/`createGoal`, `loadFeedback`/`createFeedback` all take a subject now. Task/workload signals are a natural empty/`null` for an account subject (never a task assignee, no `workloadPercent` column), the same reasoning `attendance-data.ts` already used. `recalcEmployeePerformance`/`safeRecalcEmployeePerformance` (employee-only, bare `employeeId`) were kept as thin wrappers around the new subject-shaped `recalcPersonPerformance`/`safeRecalcPersonPerformance` specifically so the 6 existing task-write call sites (`app/api/tasks/*`) needed **zero** changes — task assignees are always Employees. `recalcCompanyPerformance` now sweeps `db.companyAccount.findMany` alongside `db.employee.findMany`, so the periodic sweep (`jobs/recalculatePerformance.ts`, unchanged itself) and the goal/feedback create routes recompute a company account's score too. Routes moved from `app/api/performance/[employeeId]/*` to `app/api/performance/[memberKind]/[memberId]/*` and pages from `app/(dashboard)/performance/[id]/*` to `.../[memberKind]/[memberId]/*`, mirroring Squad's own URL shape (confirmed with the user). An employee subject keeps the existing `canEditEmployee`/`canViewPerformance` gates; a company-account subject (no manager relationship to check) uses `isCompanyAdmin` to set goals/give feedback and `isCompanyAdmin` **or self** to view — the same split Squad's account branch already used for its Attendance panel. `GoalList`/`GoalForm`/`FeedbackForm`/`GoalDecisionActions` now take a `subject` prop instead of a bare `employeeId`, building their fetch URLs as `/api/performance/${subject.kind}/${subject.id}/...`. Squad's account branch gained a "Growth" panel (score, breakdown, goals, feedback — mirroring the employee branch's, plus `GoalForm`/`FeedbackForm` for `isCompanyAdmin`). `employees/[id]/page.tsx`'s stale "Phase 8 not built yet" placeholder was replaced with a real "View performance →" link.
- **Status (21, growth parity):** Complete and verified live. `/my-space/growth` now loads `loadPerformanceBreakdown` and renders the same `BreakdownTiles` card `/performance/[memberKind]/[memberId]` shows an admin, plus a new self-service `app/(dashboard)/my-space/growth/report/page.tsx` (mirrors the admin report page, scoped to `actor` directly, no id param) reachable via a new "Download report" button on the Growth page header.
- **Verification (20 performance half + 21):** `tsc --noEmit`, `eslint`, full suite (462/462 — 4 new tests: an Owner can set a goal/give feedback to a company account, a Manager cannot) all pass. `npx next typegen` was needed once to regenerate `.next/types/validator.ts` after the route folder renames (plain `tsc` doesn't trigger this on its own). Driven against the real dev server and Postgres with curl, end-to-end, against a freshly-registered QA company with a QA Manager account and a QA Employee (direct DB inserts via a one-off `tsx` script rather than the invite flow, faster for setup): Owner created a goal + feedback for both the employee and the manager account via the new `/api/performance/{employee,account}/{id}/goals|feedback` routes (201s); a direct DB read confirmed the written rows split `employeeId`/`accountId` correctly; both `/performance/employee/{id}` and `/performance/account/{id}` pages rendered the new goal/feedback text; Squad's account page rendered the new Growth panel; the Owner decided the account's goal Achieved via `PATCH .../goals/{goalId}` (200); both admin report pages and the employee's own `/my-space/growth` + `/my-space/growth/report` rendered correctly (breakdown tiles present); a Manager was confirmed forbidden (403) from setting a goal for another company account, while able to view their own. QA company cleaned up afterward.
- **Known Turbopack-dev quirk hit this session:** a brand-new **nested** dynamic API route (`goals/[goalId]/route.ts`, one level deeper than its already-known `goals/[memberId]` parent) returned 404 even after a full dev-server restart, until the file was `touch`ed once more while the server was running — Turbopack's initial route scan appears to miss a route folder created just before the dev server's own startup scan in some cases. Not a code bug; worth remembering if a freshly-created nested route 404s right after a restart.
- **Status (18a–19):** Complete and verified live. **18a** — `navigationFor` (`lib/permissions.ts`) now gates Squad/Chat/Calendar behind `mode === "pms"` (they used to show in both modes); Employees/Performance/Requests were already HRMS-only. **18b** — the sidebar `<aside>` collapses to icon-only (`w-16`) by default and expands to `w-60` on hover (`group`/`group-hover:` pairing, `transition-[width]`), with every label (nav items, company name, mode toggle, profile — since moved, see 19 — wordmark, sign-out) wrapped to hide until hovered; `SignOutButton`/`BrandMark` gained an optional `labelClassName` prop so their other call sites (mobile topbar) are unaffected. **18c** — new `Company.brandColor` column (migration `20260916133120_phase18_brand_color`, default `#ffcc00`) is Owner-only (`canManageBranding`) and applied as an inline `--brand-yellow` CSS variable on the `.dashboard-theme` root in `app/(dashboard)/layout.tsx`; `--brand-yellow-light`/`-hover` in `app/globals.css` are now `color-mix()`-derived from it instead of hardcoded hex, so any chosen color cascades correctly, and the active sidebar item's new box-shadow (`shadow-[0_2px_10px_-2px_var(--brand-yellow)]`) follows it too. Dark theme is a **personal**, cookie-based preference (`THEME_COOKIE`/`ThemeMode` in `lib/permissions.ts`, `POST /api/settings/theme`, same shape as the pre-existing `dashboardMode` cookie) applied as `data-theme="dark"` on the same `.dashboard-theme` root, scoped by a new `.dashboard-theme[data-theme="dark"]` block in `globals.css` (black/near-black surfaces, grayish text) — never touches bare `:root`, so the marketing site is untouched. `/settings`'s page-level gate was relaxed from "Owner/Admin/Manager only" to "any signed-in actor" so employees can reach the new Appearance card too (every other card there still checks its own permission, unchanged); `navigationFor`'s employee branch gained a `/settings` link for the same reason. **19** — the profile block moved out of the sidebar into the topbar `<header>`, as the last (right-most) child, restyled compact for a horizontal row; no new dropdown, still a single `Link` to `/profile`.
- **Verification (18a–19):** `tsc --noEmit`, `eslint`, full test suite (456/456, `lib/permissions.test.ts` assertions rewritten for the new nav-gating behavior) all pass. Driven against the real dev server and Postgres with curl (dev server needed a restart after the `brandColor` migration — same recurring lesson every schema-changing session carries): registered a fresh company, confirmed HRMS-mode `/dashboard` HTML has no squad/chat/calendar links and PMS-mode has no employees/performance/requests links; toggled dark theme via `POST /api/settings/theme` and confirmed `data-theme="dark"` on the dashboard root but absent on `/` (marketing); set a custom brand color via `PATCH /api/settings/branding` as Owner (200) and confirmed the rendered `--brand-yellow` inline style and the active nav item's box-shadow both picked it up; confirmed exactly one `/profile` link in the page, inside `<header>`, after the notification bells. QA company cleaned up afterward. **Not yet checked in a real browser** (only server-rendered HTML/CSS were inspected) — the hover-expand transition itself (18b) and dark-mode visual contrast (18c) still want an actual browser pass.
...
### 2026-09-16 — Session 18

**Phase worked on:** Plan: 6 dashboard-UX/HR requests — finished Phase 20's performance half (widened `lib/performance-data.ts` + moved `/performance/[id]` to `/performance/[memberKind]/[memberId]`) and Phase 21 (growth page parity). This completes all 6 requested features. See Current Status above for full detail; not repeated here.

**What was completed:** See Current Status (the "20, performance half" and "21, growth parity" entries).

**Verification performed:** See Current Status. Full detail: `npx tsc --noEmit`, `eslint`, `npx vitest run` (462/462 across 42 files) all clean. `npx next typegen` regenerated route types after the folder renames. Curl-driven end-to-end against a freshly-registered QA company (QA Owner, a QA Manager company account, a QA Employee — the latter two inserted directly via a one-off `tsx` script rather than the invite flow) covering both an employee subject and a company-account subject through the full goal/feedback/decide/report/growth-parity flow — see Current Status for the exact assertions. QA company deleted afterward (cascades to its accounts/employees/goals/feedback/performance records).

**Files created/changed:**

- Created: `app/api/performance/[memberKind]/[memberId]/{route.ts,goals/route.ts,goals/route.test.ts,goals/[goalId]/route.ts,feedback/route.ts,feedback/route.test.ts}`, `app/(dashboard)/performance/[memberKind]/[memberId]/{page.tsx,report/page.tsx}`, `app/(dashboard)/my-space/growth/report/page.tsx`
- Deleted: `app/api/performance/[employeeId]/**`, `app/(dashboard)/performance/[id]/**`
- Changed: `lib/performance-data.ts` (full rewrite — `PerformanceSubject`, `subjectWhereFor`, every function widened, `recalcPersonPerformance`/`safeRecalcPersonPerformance` added alongside the kept employee-only wrappers, `recalcCompanyPerformance` sweeps accounts too, `goalBelongsTo` helper replacing a bare `goal.employeeId !== employeeId` check), `app/(dashboard)/performance/page.tsx` (queue link → `/performance/employee/{id}`), `app/(dashboard)/my-space/growth/page.tsx` (+breakdown card, +report link, subject-shaped calls), `app/(dashboard)/squad/[memberKind]/[memberId]/page.tsx` (account branch +Growth panel; employee branch's performance calls widened), `app/(dashboard)/employees/[id]/page.tsx` (stale Phase-8 placeholder → real performance link), `components/performance/{goal-form.tsx,feedback-form.tsx,goal-views.tsx,goal-decision-actions.tsx}` (`employeeId` prop → `subject`, fetch URLs updated)

**Known issues / TODOs:**

- Same real-browser gap as last session: the hover-expand sidebar (18b), dark mode (18c), and now the new Growth "Download report" button and Squad's account Growth panel forms have only been checked via curl/server-rendered HTML, not an actual browser. Worth a pass at some point, not blocking.
- The central `/performance` queue (HRMS employee roster) was deliberately left employee-only, not widened to list company accounts too — an Owner/Admin reviews a company account's performance from that account's own Squad profile instead (confirmed reasonable, not explicitly requested either way).

**Next step:**

- All 6 requested features are complete. If more work continues on this track, a real-browser pass (18b/18c/20/21 UI) is the one remaining verification gap, and nothing else is outstanding from the plan.

---

### 2026-09-16 — Session 17

**Phase worked on:** Plan: 6 dashboard-UX/HR requests — Phases 18a (nav gating), 18b (collapsible sidebar), 18c (theme + brand color), 19 (profile reposition), and Phase 20's attendance half (attendance/breaks for all company accounts). See Current Status above for full detail; not repeated here. Phase 20's performance half and Phase 21 are the next session's work.

**What was completed:** See Current Status (both the 18a–19 entry and the 20-attendance-half entry).

**Verification performed:** See Current Status. Full detail: `npx tsc --noEmit`, `eslint`, `npx vitest run` (458/458 across 42 files) all clean throughout. Two migrations applied against the real Supabase Postgres this session (`npx prisma migrate dev`): `20260916133120_phase18_brand_color` and `20260916134334_phase20_attendance_performance_accounts` — the dev server needed a restart after each (kept the pre-migration Prisma client and 500'd until restarted, the same recurring lesson every schema-changing session has carried since Phase 7). Curl-driven end-to-end against two freshly-registered QA companies (both deleted afterward via one-off `tsx` scripts): nav hrefs confirmed correct in both dashboard modes; theme cookie confirmed scoped to the dashboard only; brand color PATCH confirmed both the CSS variable and the active-nav box-shadow pick it up; profile link confirmed singular and positioned last in the topbar; an Owner clocked in, started and ended a break, and clocked out, with a direct DB read confirming `accountId` set and `employeeId` null on both the `AttendanceRecord` and `BreakRecord` rows; Squad's account detail page confirmed rendering "Total logged: …" for that Owner viewing their own attendance.

**Files created/changed:**

- Created: `app/api/settings/theme/route.ts`, `app/api/settings/branding/route.ts`, `components/dashboard/theme-toggle.tsx`, `components/dashboard/branding-form.tsx`, migrations `prisma/migrations/20260916133120_phase18_brand_color/` and `prisma/migrations/20260916134334_phase20_attendance_performance_accounts/`
- Changed: `lib/permissions.ts` (`navigationFor` mode-gates Squad/Chat/Calendar, employee branch +`/settings`; +`THEME_MODES`/`ThemeMode`/`THEME_COOKIE`/`canManageBranding`), `lib/permissions.test.ts` (nav-gating assertions rewritten), `lib/validations/settings.ts` (+`themeSettingsSchema`/`brandColorSchema`), `prisma/schema.prisma` (+`Company.brandColor`; +nullable `accountId`/optional `employeeId` on `AttendanceRecord`/`BreakRecord`/`PerformanceRecord`/`Goal`/`Feedback`, +5 reverse relations on `CompanyAccount`), `app/(dashboard)/layout.tsx` (sidebar collapse classes, profile moved to topbar, theme/brandColor read + applied, `BreakOverlay` no longer employee-only), `app/(dashboard)/settings/page.tsx` (page-level gate relaxed, +Appearance/Branding cards), `app/globals.css` (`--brand-yellow-light`/`-hover` now `color-mix()`-derived, +`.dashboard-theme[data-theme="dark"]` block), `components/dashboard/sidebar-nav.tsx` (label collapse span, active-item box-shadow), `components/dashboard/sign-out-button.tsx` (+`labelClassName` prop), `lib/attendance-data.ts` (dual-FK `subjectColumns`/`subjectWhereFor` helpers throughout, `loadEmployeeAttendance` renamed `loadPersonAttendance`), `lib/auth.ts` (sign-out closes a break for either actor type), `app/api/attendance/{clock-in,clock-out,break/start,break/end}/route.ts` (dropped the employee-only 403), `app/(dashboard)/dashboard/page.tsx` (+`AttendanceWidget` card), `app/(dashboard)/squad/[memberKind]/[memberId]/page.tsx` (account branch +Attendance panel), `app/(dashboard)/employees/[id]/page.tsx` (`loadPersonAttendance` rename)
- Tests: `app/api/attendance/clock-in/route.test.ts` and `.../break/start/route.test.ts` (+2: company-account clock-in and break-start coverage)

**Known issues / TODOs:**

- No real-browser check of the hover-expand sidebar transition, dark-mode visual contrast, or the break overlay's client-rendered modal (a curl fetch of server HTML can't see a Radix Dialog portal that mounts client-side) — only server-rendered HTML/CSS and API responses were inspected. Worth a real browser pass before calling 18b/18c/20 fully done.
- Dark mode leaves the status palette (success/warning/danger/info) untouched, same deliberate choice the light dashboard theme already made — flagged, not a bug.
- Phase 20's performance half is not started — see Current Status's "Not yet done" paragraph for the exact list of functions and call sites still needing the same dual-FK widening `attendance-data.ts` just got.

**Next step:**

- Finish Phase 20: widen `lib/performance-data.ts` to a `{kind, id}` subject the same way `lib/attendance-data.ts` was widened this session, move `/performance/[id]` to `/performance/[memberKind]/[memberId]` (mirroring Squad's own route shape), update every call site listed in Current Status, and add the equivalent "Growth" panel to Squad's account branch. Then Phase 21 — growth page parity — adds `loadPerformanceBreakdown` + `BreakdownTiles` to `/my-space/growth`, which depends on the performance half's widened signature.

---

### 2026-09-15 — Session 16

**Phase worked on:** Plan.md Phase 17 — Calendar (Google preview + in-app meetings)

**What was completed:**

- **Schema:** `GoogleCalendarConnection` (`employeeId`/`accountId` both individually `@unique`, same shape as `NotificationPreference` — one row per person), `Meeting` (organizer as two nullable FKs, `status: MeetingStatus { Scheduled Cancelled }`), `MeetingParticipant` (mirrors `ConversationParticipant` exactly — the organizer is also inserted as a participant row, so "my meetings" and "who is on this meeting" both read from one list). `NotificationType` widened with `MeetingScheduled`. Migration `20260915131926_phase17_calendar`.
- **Secrets (`lib/google-calendar-crypto.ts`, new, pure):** `encryptRefreshToken`/`decryptRefreshToken` — AES-256-GCM under a new `GOOGLE_TOKEN_ENCRYPTION_KEY`, random 12-byte IV per call, `iv.authTag.ciphertext` base64url-joined; a Google refresh token has to stay reversible (Google requires it back on every call), unlike the invite tokens this schema otherwise only ever hashes. `signOAuthState`/`verifyOAuthState` — HMAC-SHA256 over `{actorId, companyId, accountType, issuedAt}` under the same key, `timingSafeEqual`-compared, 5-minute expiry; this *is* the "signed state parameter (CSRF)" Plan.md asked for, carried as the OAuth `state` query param itself rather than a new cookie mechanism (no cookie precedent exists anywhere in this repo, and Google already round-trips `state` for free). `googleCalendarConfigured()` is the one function every route and the UI call to decide "is Google set up at all".
- **Google access (`lib/google-calendar.ts`, new):** plain `fetch` against Google's OAuth/Calendar REST endpoints — no `googleapis` package (Rules.md section 1, same reasoning `lib/mailer.ts`/`lib/whatsapp.ts` already follow). Access tokens are never stored; every read exchanges the stored refresh token for a fresh one first, trading a small extra round trip for zero token-expiry bookkeeping. Scope is `openid email calendar.readonly` — the `openid email` half only exists so the connect flow can learn which Google address was just connected (`fetchUserEmail` against Google's userinfo endpoint), not for any write access.
- **Pure interval logic (`lib/calendar.ts`, new):** `mergeIntervals`, `hasConflict`, `freeSlots`, `workingHoursWindow` — no such helper existed anywhere in the repo before this session. Working hours are a flat `09:00–18:00 UTC` pair of constants, company-wide and un-timezoned like every other date-only field this schema already stores — a known simplification, the one place to widen it if it matters later.
- **Privacy-enforcing data layer (`lib/calendar-data.ts`, new):** the rule Plan.md calls out — your own Google events keep titles, a colleague's Google data comes back as busy-only intervals, an in-app meeting you're on shows its title regardless — is enforced here, not in the UI or the route. `loadColleagueBusy` requires the *viewer's own* Google connection (the `freeBusy` call runs on the viewer's token, per Plan.md's confirmed design) and the colleague's `googleEmail` on file; a colleague who never connected simply previews as "no Google data", not an error. `proposeMeeting` merges the organizer's own WorkPulse meetings + Google preview via `lib/calendar.ts`, refuses a conflicting slot with the strict `WriteFailure` convention (`duplicateFailure`, matching `lib/attendance-data.ts`/`lib/task-timer-data.ts` rather than chat's looser inline variant), creates the `Meeting` + every `MeetingParticipant` row in one `db.create`, and notifies every invitee but never the organizer (the same "don't notify the person who just did it" rule `notifyTaskAssigned` already follows).
- **New `canCancelMeeting`** in `lib/permissions.ts` — organizer-only, no lead/admin escape hatch (nobody has been given the power to cancel someone else's meeting for them).
- **Routes:** `GET/POST /api/meetings`, `DELETE /api/meetings/[id]`, `GET /api/calendar/availability`, `GET /api/calendar/google/{connect,callback,disconnect}`. A missing Google client is a typed 503 `not_configured`, never a 500 — the connect route, the callback (which redirects to `/calendar?error=...` on any failure rather than throwing), and the `/calendar` page's own connect card all check `googleCalendarConfigured()` the same way.
- **UI:** new `/calendar` page (added to both `PROTECTED_PREFIXES`/`SHARED_PREFIXES` in `proxy.ts` and to `navigationFor` in both branches, same as Squad/Chat), mirroring `/chat`'s server-auth-then-client-shell split rather than `/squad`'s fully server-rendered page — `components/calendar/{calendar-shell,connect-google-card,propose-meeting-form}.tsx`. The shell polls `/api/meetings` every 30s (same precedent `ConversationList` already set), a colleague picker overlays busy blocks from `/api/calendar/availability`, and a propose-meeting form books straight into WorkPulse.
- **Notifications:** `MeetingScheduled` added to `CHANNELS_BY_TYPE` (`InApp`/`Push`/`Email`/`WhatsApp` — a direct personal invite, the same weight as `TaskAssigned`) and `EMAIL_SUBJECTS`; `notifyMeetingScheduled` in `lib/notification-data.ts` follows `notifyTaskAssigned`'s exact shape.
- **Tested:** `lib/calendar.test.ts` (+14: interval merge/conflict/free-slot edge cases), `lib/google-calendar-crypto.test.ts` (+9: encrypt/decrypt round-trip and tamper detection, OAuth state sign/verify including expiry and actor-mismatch rejection, `googleCalendarConfigured`), `lib/validations/calendar.test.ts` (+5: `proposeMeetingSchema`), two new route-test files (`app/api/meetings/route.test.ts` +4, `app/api/calendar/google/connect/route.test.ts` +2 — only the "not configured" path is testable without real Google credentials, matching Phase 13's own precedent). Two pre-existing `lib/permissions.test.ts` `navigationFor` assertions updated for the new `/calendar` nav item. 18 new tests; **449/449 passing**.

**Verification performed:**

- `npx prisma migrate dev` applied the new tables against the real Supabase Postgres; `npx prisma generate`, `npx tsc --noEmit`, `eslint .`, `npm run build`, `npm test` (449/449) all clean.
- **Driven against the running dev app and the real Postgres with curl** (dev server restarted first for the fresh Prisma client, same recurring lesson every schema-changing phase since 7 has carried): registered a fresh company, created a Manager account and an employee, then as the Owner: booked a meeting with the employee → 201, `GET /api/meetings` showed it with both participants named; proposing an overlapping slot for the same organizer → 409 `duplicate`; `DELETE /api/meetings/[id]` by the organizer → 200, and the meeting then read back with `status: "Cancelled"`. `GET /api/calendar/google/connect` with no `GOOGLE_*` env vars → 503 `not_configured`; `GET /calendar` → 200 and the HTML confirmed to contain "Connect Google Calendar" alongside "Not set up for this app yet" (the disabled-state copy), not a broken page.
- **Explicitly not verified this session:** the real Google OAuth handshake end-to-end (consent screen, token exchange, `events.list`, `freeBusy`) — no Google Cloud project or client credentials exist in this environment. This is the exact "blocked on the user" state Plan.md itself predicted for this phase; the same class of gap Phase 13's never-configured WhatsApp/Push providers carried.
- No real-browser check of `/calendar`'s UI — same gap every phase since 6 has carried, now covering the week view, the colleague-picker availability overlay, and the propose-meeting form's checkbox list.

**Files created/changed:**

- Created (lib): `google-calendar-crypto.ts`, `google-calendar.ts`, `calendar.ts`, `calendar-data.ts`, `validations/calendar.ts`
- Created (tests): `lib/calendar.test.ts`, `lib/google-calendar-crypto.test.ts`, `lib/validations/calendar.test.ts`, `app/api/meetings/route.test.ts`, `app/api/calendar/google/connect/route.test.ts`
- Created (API): `app/api/meetings/route.ts`, `app/api/meetings/[id]/route.ts`, `app/api/calendar/availability/route.ts`, `app/api/calendar/google/{connect,callback,disconnect}/route.ts`
- Created (pages/components): `app/(dashboard)/calendar/page.tsx`, `components/calendar/{calendar-shell,connect-google-card,propose-meeting-form}.tsx`
- Changed: `prisma/schema.prisma` (+`GoogleCalendarConnection`/`Meeting`/`MeetingParticipant`/`MeetingStatus`, +`NotificationType.MeetingScheduled`, + migration), `lib/permissions.ts` (+`canCancelMeeting`, `navigationFor` +Calendar in both branches), `proxy.ts` (+`/calendar` in both prefix lists), `lib/notifications.ts`/`lib/notification-data.ts` (+`MeetingScheduled` channel/subject/message, +`notifyMeetingScheduled`), `components/dashboard/sidebar-nav.tsx` (+`CalendarDays` icon), `.env.example` (+Google Calendar block), `lib/permissions.test.ts` (2 `navigationFor` assertions updated for the new nav item)

**Known issues / TODOs:**

- **The Google OAuth handshake itself is unverified** — see Verification above. Once the user supplies a Google Cloud OAuth client (`GOOGLE_CLIENT_ID`/`SECRET`/`REDIRECT_URI` + a generated `GOOGLE_TOKEN_ENCRYPTION_KEY`), the connect → consent → callback → preview round trip still needs a real run.
- **No real-browser check of `/calendar`'s UI**, see Verification above.
- Working hours for `freeSlots` are a flat `09:00–18:00 UTC`, not per-company/per-timezone — Plan.md didn't ask for either; `lib/calendar.ts`'s two constants are the one place to widen this.
- `queryFreeBusy` (colleague preview) depends on Google's own free/busy sharing between the two Google accounts — a colleague outside the viewer's Workspace domain who hasn't explicitly shared their calendar will preview as "no busy data" rather than erroring, which is the documented graceful degradation, not a bug, but worth knowing before assuming the preview is authoritative.

**Next step:**

- This was Plan.md's last named phase (13→14→15→16→17, all now built). Before calling v1 fully done: get real Google Cloud credentials into `.env.local` and run the OAuth handshake live; the real-browser UI pass carried since Phase 6 (now covering five more phases' worth of UI); and the still-open Phase 0 hosting-deployment blocker.

---

### 2026-09-15 — Session 15

**Phase worked on:** Plan.md Phase 16 — half day on a leave request

**What was completed:**

- **Schema:** `enum LeaveDayPart { FullDay FirstHalf SecondHalf }` and a nullable `Request.dayPart LeaveDayPart?` — the same "nullable column used only by the types that need it" shape `startDate`/`endDate`/`amount` already carry, per Plan.md's own design, rather than a second table. Two migrations: `20260915122616_phase16_leave_day_part` (the column) and a separate `20260915122700_phase16_backfill_leave_day_part` (`UPDATE "Request" SET "dayPart" = 'FullDay' WHERE "type" = 'Leave' AND "dayPart" IS NULL`) — split because the column migration was already applied against the real Supabase Postgres by the time the backfill was written, so the backfill had to be a new migration rather than an edit to an applied one's SQL.
- **`lib/requests.ts`** (pure, extended): `requestNeedsDayPart(type)` — `Leave` only, beside the existing `requestNeedsDateRange`/`requestNeedsAmount`, so the form and the write resolver keep agreeing about what a type needs (widening to `WFH` later is one `||`, per Plan.md's own open decision). `LEAVE_DAY_PARTS`, `dayPartLabel`, and `requestTypeDisplay(type, dayPart)` — the last one is the single place that renders "Leave · First half" (a `FullDay` or absent day part shows just the type), shared by the form isn't needed here but the list and detail views are, so they can never disagree.
- **`lib/validations/requests.ts`:** `dayPart` is optional in the schema (not `.default()` — an earlier attempt at `z.enum(...).default("FullDay")` broke `useForm<CreateRequestInput>`'s type inference against `zodResolver`, since a defaulted field's zod input/output types diverge and `CreateRequestInput` is one type used for both; reverted to plain `.optional()` and pushed the default down into `resolveRequest` instead). The existing `superRefine` gained one more check: when the type needs a day part and it's a half (not `FullDay`), `startDate` must equal `endDate`.
- **`lib/request-data.ts`:** `RequestWriteData`/`LoadedRequest` gain `dayPart`; `requestSelect` selects it; `resolveRequest` writes `input.dayPart ?? "FullDay"` for a type that needs one, `null` otherwise — this is where the "defaults to FullDay" behavior actually lives, not the schema.
- **UI:** `request-form.tsx` shows a "Day part" select (Full day/First half/Second half) when `requestNeedsDayPart(type)`, with a hint that a half day must be a single day. `request-detail.tsx`'s page header and `request-views.tsx`'s Type column both switched from `requestTypeLabel` to the new `requestTypeDisplay(type, dayPart)`.
- **The three request-detail pages** (`app/(dashboard)/requests/[id]/page.tsx`, `app/(dashboard)/my-space/requests/[id]/page.tsx`, `app/(dashboard)/my-space/requests/approvals/[id]/page.tsx`) each keep their own `requestDetailSelect` object rather than importing `lib/request-data.ts`'s — all three needed `dayPart: true` added by hand, caught immediately by `tsc` once `LoadedRequest` required the field.
- **Tested:** `lib/requests.test.ts` (+4: `requestNeedsDayPart`, `dayPartLabel`, `requestTypeDisplay`), a new `lib/validations/requests.test.ts` (+5: the schema had no test file before this session — optional-not-defaulted, full day and same-day-half accepted, a multi-day half refused with the error on `endDate`, a non-Leave type's stray `dayPart` ignored) and a new `lib/request-data.test.ts` (+3: `resolveRequest` is pure — no DB access despite living in a `-data.ts` file — so it's unit-tested directly like `lib/requests.ts`'s predicates: defaults to `FullDay`, keeps an explicit half, nulls it out for a type that doesn't need one). 12 new tests; **408/408 passing**.

**Verification performed:**

- `npx prisma migrate deploy` applied both migrations against the real Supabase Postgres (`DATABASE_URL` from `.env.local`); `npx prisma generate`, `npx tsc --noEmit`, `eslint .`, `npm run build`, `npm test` (408/408) all clean.
- **Driven against the running dev app and the real Postgres with curl** (the dev server needed a restart first for the fresh Prisma client — same lesson every schema-changing phase since 7 has carried; `POST /api/requests` for a half-day Leave 500'd with the pre-restart stale client and succeeded immediately after): registered a fresh company and employee, then confirmed all four cases — a single-day `FirstHalf` Leave → 201; a `SecondHalf` spanning two days → 400 `validation_error` naming `endDate`; a Leave with no `dayPart` at all → 201, stored as `FullDay`; a `Reimbursement` carrying a stray `dayPart: "FirstHalf"` → 201, stored with `dayPart: null`. Rendering confirmed by fetching real pages with the session cookie: the employee's own request detail and "My Requests" list both show "Leave · First half" for the half day and plain "Leave"/"Reimbursement" for the other two; the Owner's `/requests` approval queue shows the same "Leave · First half"; `/my-space/requests/new` renders the "Day part" select with all three options.
- **Backfill checked directly against the database** with a `tsx --env-file=.env.local` script (not committed): of 26 pre-existing `Leave` rows across every company in this dev database, **0** had a null `dayPart` after the backfill migration, and **0** non-`Leave` rows had one set.

**Files created/changed:**

- Created (migrations): `prisma/migrations/20260915122616_phase16_leave_day_part/`, `prisma/migrations/20260915122700_phase16_backfill_leave_day_part/`
- Created (tests): `lib/validations/requests.test.ts`, `lib/request-data.test.ts`
- Changed: `prisma/schema.prisma` (+`LeaveDayPart` enum, +`Request.dayPart`), `lib/requests.ts` (+`requestNeedsDayPart`/`LEAVE_DAY_PARTS`/`dayPartLabel`/`requestTypeDisplay`), `lib/requests.test.ts`, `lib/validations/requests.ts` (+`dayPart` field and the single-day refinement), `lib/request-data.ts` (+`dayPart` on `RequestWriteData`/`LoadedRequest`/`requestSelect`, `resolveRequest`'s default), `components/requests/request-form.tsx` (+"Day part" select), `components/requests/request-detail.tsx`, `components/requests/request-views.tsx` (both switched to `requestTypeDisplay`), `app/(dashboard)/requests/[id]/page.tsx`, `app/(dashboard)/my-space/requests/[id]/page.tsx`, `app/(dashboard)/my-space/requests/approvals/[id]/page.tsx` (each `requestDetailSelect` +`dayPart: true`)

**Known issues / TODOs:**

- **No real-browser check** — same gap carried since Phase 6, now covering the new "Day part" select's conditional visibility too.
- Plan.md's own open decision — half days are `Leave` only, `WFH` being a one-line widening (`requestNeedsDayPart`'s one `||`) — left as recommended, not built.

**Next step:**

- Plan.md Phase 17 — Calendar (Google preview + in-app meetings). The largest of the five phases, and the only one needing something from the user first: a Google Cloud project with the Calendar API enabled, an OAuth consent screen, and a client ID/secret.
